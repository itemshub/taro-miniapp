...
const Market = () => {
  ...
  const filterSkins = (
    ...
    let filtered = indexData.markets;
  ...
  return (
    <View className="market-page">
      ...
      <View className="results-count">
        <View className="count-text">找到 {skins.length} 个平台</View>
      </View>
      ...
      <ScrollView scrollY className="skins-list">
        {cases.length === 0 ? (
          <View className="empty-state">
            <View className="empty-text">没有找到匹配的平台</View>
          </View>
        ) : (
            ...
            {markets.map((market:any) => (
              ...
              >
                ...
                <View className="list-item-content">
                  <View className="list-item-name">{market.name}</View>
                  {/* <View className="list-item-skin">{market.id}</View> */}

                  <View className="list-item-info">
                    <View
                      className={`list-item-change ${
                        market?.avg_discount >= 0 ? 'positive' : 'negative'
                      }`}
                    >
                      <View
                        className={
                          market?.avg_discount >= 0
                            ? 'icon-trending-up-small'
                            : 'icon-trending-down-small'
                        }
                      />
                      平均低于Steam : {(market?.avg_discount).toFixed(1)}%
                    </View>
                  </View>

                  <View className="list-item-name">
                    平台挂单数 : {market.offers.toFixed(0)}
                  </View>
                  <View className="list-item-name">
                    月均访问 : {market.monthly_visits}
                  </View>
                </View>
